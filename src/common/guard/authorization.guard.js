  ...
  const Authorization = async (req, res, next) => {
    try {
        ...
        const [bearer, token] = authorization?.split(" ");
         console.log('Token:', token); // لاگ اضافه شده 
        if (!token) throw new createHttpError.Unauthorized(AuthorizationMessage.Login);
        const data = jwt.verify(token, process.env.JWT_SECRET_KEY);
        if (typeof data === "object" && "id" in data) {
            const user = await UserModel.findById(data.id, { refreshToken: 0}).lean();
...
/*  const Authorization = async (req, res, next) => {
    try {
        ...
        console.log(typeof req.cookies); // باید 'string' باشد
        console.log(req.cookies); // نمایش توکن
...
/*   const Authorization = async (req, res, next) => {
    try {
        const accessTokenObject = req?.cookies?.access_token;


        // بررسی اینکه آیا access_token به درستی وجود دارد
        if (!accessTokenObject || !accessTokenObject.accessToken) {
            throw new createHttpError.Unauthorized(AuthorizationMessage.Login);
        }

        // استخراج accessToken از آبجکت
        const token = accessTokenObject.accessToken;
